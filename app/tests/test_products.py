...
def test_crud_flow(client):
    ...
    read_resp = client.get(f"/prodcuts/{product_id}")
    assert read_resp.status_code == 200
    
    update_payload = {"name" : "prod2", "stock" : 7, "price" : 12.5}
    update_resp = client.put(f"/products/{product_id}", json = update_payload)
    assert update_resp.status_code == 200
    update = update_resp.json()
    assert update["stock"] == 7
...
def test_duplicate_name_returns_404(client):
    ...
    assert resp1.json().get("detail") == "Producto ya existe"
